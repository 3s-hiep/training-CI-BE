import { CIEBackendError, ErrorCode } from "./error";

describe("CIEBackendError", () => {
  const errorClassName = "CIEBackendError";

  describe("isCIEBackendError", () => {
    [
      { title: "object created via new", input: new CIEBackendError(0, "message"), expected: true },
      {
        title: "object created via new with original error",
        input: new CIEBackendError(1, "msg", new Error("original")),
        expected: true,
      },
      ...
      it(`should return ${testCase.expected} when input is ${testCase.title}`, () => {
        ...
        // act
        const actual = CIEBackendError.isCIEBackendError(testCase.input);
  ...
  describe("toString", () => {
    it("should return formatted string", () => {
      // arrange
      const target = new CIEBackendError(1, "message", { error: "original" });
...
describe("ErrorCode", () => {
  describe("categorize()", () => {
    it("should return argument as-is if argument is instance of AssetCIEBackendError", () => {
      // arrage
      const expected = new CIEBackendError(ErrorCode.NOT_FOUND, "asset service error des");
